...
var maxAliensX = 8;
var curXPosition = 0;
// The maximum X position is calculated by the number of aliens wide
var aliensTotalWidth = 0;
// The pixels per tick the aliens move left or right
var movementSpeed = 1;

var ticktocksprite = 0;
...
function drawAlien(alienX, alienY, name) {
    ...
    if (contains(nodes, name)) {
        ...
        if (ticktocksprite == 0) {
            ...
            image.src = './images/sprite_invader2.png';
        }
        ctx.font = '8px pixel';
        ctx.drawImage(image, alienX, alienY, 40, 40);
        if (showPodName) {
            ctx.fillText(name.substring(0, 20), alienX, alienY + 40);
        }

        // Display the infra logo
        var image2 = new Image();
        image2.src = './images/' + name + '.png';
        ctx.drawImage(image2, alienX, alienY, 20, 20);
    }
    ctx.closePath();
}
...
function checkRocketAlienCollision() {
    if (contains(aliensY, rocketY)) {
        ...
        for (i=aliens.length - 1; i >= 0; i--) {
            if (aliens[i]["active"] && (aliens[i]["dying"]===0) && (rocketY - aliens[i]["y"] < 5)) {
                var rangeX = []
                rangeX.push(aliens[i]["x"] + curXPosition);

                for (k=aliens[i]["x"]; k<aliens[i]["x"]+aliensWidth; k++) {
                    rangeX.push(k + curXPosition);
                }
                ...
                if(contains(rangeX, rocketX)) {
                    ...
                    if (contains(nodes, aliens[i]["name"])) {
                        ...
                        console.log("[Killed] " + aliens[i]["name"]);
...
window.setInterval(function draw() {
    ...
    if(upPressed) {
        ...
        // Limit the ship to bottom half of the screen
        if (spaceshipY < canvas.height/2) {
    ...
    // Calculate alien lateral movement
    curXPosition += movementSpeed;

    // We need to change direction if we run off either end of the canvas
    if (curXPosition <= 0 ) {
        movementSpeed = 1;
    }
    if (curXPosition + aliensTotalWidth > canvas.width) {
        movementSpeed = -1;
    }

    
    for (i=0; i<aliens.length; i++) {
        if (aliens[i]["active"] && (aliens[i]["dying"] === 0)) {
            drawAlien(aliens[i]["x"] + curXPosition, aliens[i]["y"], aliens[i]["name"]);
        } else {
            //console.log("Not Drawing: " + i + " / " + aliens[i]["active"] + " / " + aliens[i]["dying"])
        }
    }
    ctx.fillStyle = 'white';
    ctx.font = '16px pixel';
    ...
    if (pods.length > 0) {
        ...
        for (j=0; j<pods.length; j++) {
            ...
            if(!podExists(podName)) {
                ...
                if (replaceWith != -1) {
                    aliens[replaceWith] = {"name": podName, "x": aliens[replaceWith]["x"], "y": aliens[replaceWith]["y"], "active": true, "dying": dying}
                    cnt =+ 1;
                }
                else {
                    if (!yInc) {
                    ...
                    aliens.push({"name": podName, "x": x, "y": y, "active": true, "dying": dying});
                    cnt =+ 1;
                }
                if (aliens.length % maxAliensX == 0) {
                    x = 10;
                    y += aliensIncrementY;
                    for (k=y+10; k>=y; k--) {
                        if (!contains(aliensY, k)) {
...
//        }
        ...
        for (i=0; i<aliens.length; i++) {
            if (aliens[i]["dying"] > 0) {
                aliens[i]["dying"] = aliens[i]["dying"] - 1;

                // If we've now hit 0, then wipe out the name so that this slot can be re-used
                if (aliens[i]["dying"] === 0) {
                    aliens[i]["name"] = "killed_pod";
                }
            }
        }
        if (aliens.length > maxAliensX) {
            aliensTotalWidth = maxAliensX * 60;
        } else {
            aliensTotalWidth = aliens.length * 60;
        }
    }
}, 1000)
...
ws_log.onmessage = function(event) {
  var logs = document.getElementById("logging");
